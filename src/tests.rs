...
fn error_max_number_bets() {
	new_test_ext().execute_with(|| {
		...
		Betting::bet(RuntimeOrigin::signed(2), match_id, 100, MatchResult::Team1Victory).ok();
		Betting::bet(RuntimeOrigin::signed(3), match_id, 100, MatchResult::Team2Victory).ok();
...
fn set_result_of_match() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Betting::get_results(match_id).unwrap(), MatchResult::Team1Victory);
	});
}
...
fn error_set_result_no_root() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Betting::set_result(RuntimeOrigin::signed(2), match_id, MatchResult::Team1Victory),
			BadOrigin
		);
	});
}
...
fn distribute_winnings() {
	new_test_ext().execute_with(|| {
		...
		let match_id = ensure_signed(RuntimeOrigin::signed(1)).unwrap();
		assert_ok!(Betting::bet(RuntimeOrigin::signed(2), match_id, 10, MatchResult::Team1Victory));
